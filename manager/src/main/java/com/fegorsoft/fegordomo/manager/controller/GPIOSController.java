...
public class GPIOSController {
        ...
        public ResponseEntity<GPIO> add(@Parameter(description = "Name of GPIO") @RequestBody @Valid GPIODTO gpioDTO)
                ...
                GPIO gpio = new GPIO();
                Device device = deviceRepository.findById(gpioDTO.getDeviceId())
                ...
                try {
                        ...
                        ScheduleGPIO scheduleGPIO = new ScheduleGPIO(gpio.getId(), gpio.getGpio(),
                                        device.getIp().getHostAddress(), gpio.getCronTriggerOn(),
                                        gpio.getCronTriggerOff(), true);
        ...
        @ApiResponses(value = { @ApiResponse(responseCode = "201", description = "gpio updated", content = {
                        @Content(mediaType = "application/json", schema = @Schema(implementation = GPIO.class)) }),
                        @ApiResponse(responseCode = "404", description = "Bad request", content = @Content) })
        @PutMapping(path = "/{id}")
        ...
        @ApiResponses(value = { @ApiResponse(responseCode = "200", description = "Found the GPIO", content = {
                        @Content(mediaType = "application/json", schema = @Schema(implementation = GPIO.class)) }),
        ...
        @ApiResponses(value = { @ApiResponse(responseCode = "200", description = "Found GPIOs", content = {
                        @Content(mediaType = "application/json", schema = @Schema(implementation = GPIO.class)) }),
                        @ApiResponse(responseCode = "400", description = "Invalid id supplied", content = @Content),
