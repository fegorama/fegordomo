...
void utilBlink(AsyncWebServerRequest * request, uint8_t *data, size_t len, size_t index, size_t total)
{ 
  ...
  digitalWrite(gpio, HIGH);   // turn the LED on (HIGH is the voltage level)
  delay(wait);                       // wait for a second
  digitalWrite(gpio, LOW);    // turn the LED off by making the voltage LOW
  delay(wait);                       // wait for a second
